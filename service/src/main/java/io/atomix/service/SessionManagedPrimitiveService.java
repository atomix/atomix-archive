...
import io.atomix.service.impl.DefaultServiceExecutor;
import io.atomix.service.impl.DefaultServiceScheduler;
import io.atomix.service.impl.OperationCodec;
import io.atomix.service.impl.ServiceCodec;
...
import io.atomix.service.protocol.SessionCommandResponse;
import io.atomix.service.protocol.SessionManagedServiceSnapshot;
import io.atomix.service.protocol.SessionQueryRequest;
import io.atomix.service.protocol.SessionQueryResponse;
...
import io.atomix.service.protocol.SessionSnapshot;
import io.atomix.service.protocol.SessionStreamContext;
import io.atomix.service.protocol.SessionStreamResponse;
import io.atomix.service.protocol.SessionStreamSnapshot;
import io.atomix.service.session.Session;
import io.atomix.service.session.SessionId;
import io.atomix.service.session.SessionStreamHandler;
import io.atomix.service.session.StreamId;
import io.atomix.service.session.impl.PrimitiveSession;
import io.atomix.service.session.impl.PrimitiveSessionStream;
...
public abstract class SessionManagedPrimitiveService extends AbstractPrimitiveService implements PrimitiveService {
  ...
  private CompletableFuture<SessionResponse> applySessionRequest(Command<SessionRequest> command) {
    if (command.value().hasCommand()) {
      ...
      return Futures.exceptionalFuture(new ServiceException.ApplicationException());
    }
  }
  ...
  private CompletableFuture<KeepAliveResponse> applyKeepAlive(Command<KeepAliveRequest> keepAlive) {
    ...
    if (session == null) {
      return Futures.exceptionalFuture(new ServiceException.UnknownSession());
    }
