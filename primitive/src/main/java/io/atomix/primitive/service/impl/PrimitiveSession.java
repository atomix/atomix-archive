...
public class PrimitiveSession extends AbstractSession {
   ...
   */
  public PrimitiveServiceContext getService() {
    return context;
  }
   ...
   */
  private void publish(io.atomix.primitive.service.PrimitiveEvent event) {
  ...
  private void sendEvents(EventHolder event) {
    ...
    if (context.role() == Role.PRIMARY) {
      eventExecutor.execute(() -> {
        EventRequest request = EventRequest.newBuilder()
            .setServerId(context.localMemberId().id())
        ...
        clusterCommunicator.unicast(eventSubject, request, EventRequest::toByteArray, memberId());
      });
    }
  }
  ...
  public void open() {
    setState(State.OPEN);
    clusterCommunicator.subscribe(resetSubject, bytes -> {
      try {
        return ResetRequest.parseFrom(bytes);
      } catch (InvalidProtocolBufferException e) {
        throw new IllegalArgumentException(e);
      }
    }, request -> resendEvents(request.getEventIndex()), context.context());
  }
  ...
  public void expire() {
    ...
    clusterCommunicator.unsubscribe(resetSubject);
  }
  ...
  private static class EventHolder {
    ...
    private final List<io.atomix.primitive.service.PrimitiveEvent> events = new LinkedList<>();
