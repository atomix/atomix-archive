 ...
 * cluster membership information in order to perform communication. Each server must be provided a local node
 * to which to bind the internal {@link io.atomix.raft.protocol.RaftServerProtocol} and a set of addresses
 ...
 * relation to commands and queries submitted to the server by a client. State machines
 ...
 *   }
 * </pre>
 * Server state machines are responsible for implementing commands which can be
 ...
 * <h2>Storage</h2>
 * As commands are received by the server, they're written to the Raft
 ...
 * {@link #join(String...) joined} to an existing cluster. The simplest way to bootstrap a new cluster is to bootstrap
 ...
 * {@link #bootstrap(String...)} method on each server. When bootstrapping a multi-node cluster, the bootstrap configuration
 ...
 * add additional nodes to that server. Servers can join existing bootstrapped clusters using the {@link #join(String...)}
...
public interface RaftServer {
  ...
  static Builder builder() {
    try {
      ...
      return builder(address.getHostName());
   ...
   * <p>
   * The provided server ID is the address to which to bind the server being constructed.
   *
   * @param localServerId The local node identifier.
   * @return The server builder.
   */
  static Builder builder(String localServerId) {
    return new DefaultRaftServer.Builder(localServerId);
  }
   ...
   * {@link io.atomix.raft.storage.snapshot.SnapshotStore snapshot},
   * and {@link io.atomix.raft.storage.system.MetaStore configuration} files stored on disk use
   ...
   * the server is {@link #bootstrap() started}, the cluster configuration will be initialized using the server
   * list provided to the server {@link #builder(String) builder}. For {@link StorageLevel#DISK persistent}
   ...
   * however, that users need not explicitly {@link RaftCluster#join(String...) join} or {@link RaftCluster#leave() leave} the
   ...
   * @return A completable future to be completed once the cluster has been bootstrapped.
   */
  default CompletableFuture<RaftServer> bootstrap(String... members) {
   ...
   */
  default CompletableFuture<RaftServer> join(String... members) {
   ...
   * @return A completable future to be completed once the local server has joined the cluster.
   */
  CompletableFuture<RaftServer> join(Collection<String> members);
   ...
   */
  default CompletableFuture<RaftServer> listen(String... cluster) {
   ...
   * @return A completable future to be completed once the local server has joined the cluster as a listener.
   */
  CompletableFuture<RaftServer> listen(Collection<String> cluster);
  ...
  abstract class Builder implements io.atomix.utils.Builder<RaftServer> {
     ...
     * @return the Raft server builder
     */
    public Builder withStateMachine(RaftStateMachine stateMachine) {
